...
class Service extends MY_Controller 
{
            ...
            foreach ($all_vehicle as $vehicle) {
                ...
                if ($vehicle->status == 'active') {
                    ...
                    if (in_array($vehicle->id, $available_vehl)) {
                        ...
                        $buzy_in = 'busy in ' . $obj->service_title;
                        $bgcolor = '#FF0000';
                    }

                    $data[] = array(
                        'title' => $vehicle->regn_number . ' (' . $buzy_in . ')',
                        'start' => date("c", strtotime($service_date . ' ' . (isset($obj->start_time) ? $obj->start_time : '00:00:00'))),
                        'end' => date("c", strtotime($service_date . ' ' . (isset($obj->end_time) ? $obj->end_time : '00:00:00'))),
    ...
    */
    public function edit($edit_id) 
    {
        ...
        $this->form_validation->set_rules('service_title', 'Service Title', 'required|max_length[100]|callback_unique_service[services.service_title.' . $edit_id . ']');
        ...
        $this->form_validation->set_rules('service_desc', 'Description', 'required|max_length[500]');
    ...
    */
    private function validate_form() 
    {
        if ($this->input->post()) {
            ...
            $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
            $this->form_validation->set_rules('service_title', 'Service Title', 'required|max_length[100]|callback_unique_service_add[services.service_title.' . $this->input->post('service_title') . ']');
            ...
            $this->form_validation->set_rules('vehicles', 'Vehicle', 'required|max_length[100]');
    ...
    public function service_detail_ajax($service_id) 
    {
        ...
        if (is_array($services)) {
            ...
            foreach ($services as $row) {
                ...
                $actions = '<a title="Delete" class="btn btn-outline btn-circle dark btn-sm red" onClick="if(confirm(\'Do you want to delete this reocrd ?\')){window.location.href=\'' . site_url('admin/service/deleteDetail/' . $row->id . '/' . $service_id) . '\';}" href="javascript:;"><i class="fa fa fa-trash"></i> Delete</a>';
    ...
    */
    public function service_ajax() 
    {
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'filter') {
            ...
            if (isset($_REQUEST['filter_date_from']) && ($_REQUEST['filter_date_from'] != '')) {
                $filter['b.created_at'] = date("Y-m-d", strtotime(str_replace('/', '-', $_REQUEST['filter_date_from'])));
            }
        }
        ...
        if (is_array($services)) {
            ...
            foreach ($services as $row) {
                ...
                $actions .= '<a title="Delete" class="btn btn-outline btn-circle dark btn-sm red" onClick="if(confirm(\'Do you want to delete this reocrd ?\')){window.location.href=\'' . site_url('admin/service/delete/' . $row->id . '') . '\';}" href="javascript:;"><i class="fa fa fa-trash"></i> Delete</a>';

                $records["data"][] = array($chkbox, $row->service_title, $row->project, display_date($row->start_date), display_date($row->end_date), display_time($row->start_time), display_time($row->end_time), $row->department, display_datetime($row->created_at), $actions);
            }
        }
            ...
            if (is_array($employees)) {
                ...
                foreach ($employees as $row) {
                    ...
                    if ($row->status == 'active') {
                        ...
                        $avail = (in_array($row->id, explode(",", $avail_employees))) ? 'Available' : 'Buzy';
            ...
            if (is_array($vehicles)) {
                ...
                foreach ($vehicles as $row) {
                    ...
                    if ($row->status == 'active') {
                        ...
                        $avail = (in_array($row->id, explode(",", $avail_vehicles))) ? 'available' : 'buzy';
                        $options[$row->id] = $row->model . ' ( ' . $row->regn_number . ' ) - ' . $avail;
                    }
                }
            }
