...
class Department extends MY_Controller 
{
    ...
    {        
        if($this->input->post()){
            ...
            $this->form_validation->set_rules('name', 'Department Name', 'required|min_length[2]|max_length[50]|is_unique[department.name]');
    ...
    public function edit($id) 
    {
        ...
        if($this->input->post()){
            ...
            $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
            $this->form_validation->set_rules('name', 'Department Name', 'required|min_length[2]|max_length[50]|callback_unique_department[department.name.'.$id.']');
            $this->form_validation->set_rules('description', 'Description', 'max_length[500]');
            $this->form_validation->set_rules('status', 'Status', 'required');
     ...
     * @return bool
     */
    public function unique_department($value, $params)
    {   
        $this->form_validation->set_message('unique_department', 'The %s "'.$value.'" is already exists.');
    ...
    public function department_ajax()
    {
            ...
            if(is_array($services)) {
                ...
                foreach ($services as $row){
                    ...
                    $records["data"][] = array($chkbox, code($row->id), $row->name, $row->description, $row->status, display_datetime($row->created_at), $actions);
                }
            }
